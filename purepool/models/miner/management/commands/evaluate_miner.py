...
class Command(TaskCommand):
    ...
    def handle(self, *args, **options):
        ...
        days_dt = timezone.now() - datetime.timedelta(days=3)
        ...
        # 0 is the default
        miners = Solution.objects.filter(network=network, miner__enabled=True, inserted_at__gte=days_dt).values('miner__address', 'miner_id').annotate(total=Count('miner_id'))
        ...
        working_miners = Block.objects.filter(network=network, pool_block=True, inserted_at__gte=days_dt).values('miner_id').annotate(total=Count('miner_id'))
        miner_ids = []
        for working_miner in working_miners:
            miner_ids.append(working_miner['miner_id'])
        pool_solution_count = Solution.objects.filter(network=network, inserted_at__gte=days_dt, miner_id__in=miner_ids).count()
        ...
        # and now per miner
        for miner in miners:
            miner_blocks = Block.objects.filter(network=network, miner_id=miner['miner_id'], inserted_at__gte=days_dt).count()
            miner_solutions_count = Solution.objects.filter(network=network, miner_id=miner['miner_id'], inserted_at__gte=days_dt).count()

            try:
                miner_ratio = miner_solutions_count / miner_blocks
            except ZeroDivisionError:
                miner_ratio = miner_solutions_count

            percent_diff = (100 / ratio) * miner_ratio
            ...
            if miner_blocks > 5 and percent_diff < 80:
                rating = -1

            if miner_blocks > 5 and percent_diff < 70:
                rating = -2

            if miner_blocks > 5 and percent_diff < 60:
                rating = -3

            #if miner_blocks > 5 and percent_diff < 40:
            #    rating = -4

            # special for small miners: if they hadn't found a block, but
            # have a very small percent_diff, we set the rating to 0
            if miner_blocks == 0 and percent_diff < 100:
                rating = 0

            if settings.TASK_DEBUG:
                print(" - ", miner['miner__address'], miner_solutions_count, miner_blocks, miner_ratio, percent_diff, ' # ', rating)

            m = Miner.objects.get(pk=miner['miner_id'])
            m.rating = rating
            m.ratio = miner_ratio
            m.percent_ratio = percent_diff
            m.save()
